    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className={cn(
          ...
          <FolderIcon className={cn('w-7 h-7', colors.bg.replace('bg-', 'text-'))} />
        </div>
      ...
      {folder.topics.length > 0 && (
        <div className="space-y-1">
          {folder.topics.slice(0, 3).map((topic, idx) => (
            <div
              key={idx}
              className="text-xs text-slate-400 dark:text-slate-500 truncate flex items-center gap-1"
            >
...
export function UncategorizedCard({ topicCount, onSelect }: UncategorizedCardProps) {
  return (
    <div
      ...
      className={cn(
        'relative group cursor-pointer',
        'bg-slate-50 dark:bg-slate-900 rounded-2xl',
    ...
    >
      <div className="flex items-start mb-4">
        <div className="w-14 h-14 rounded-xl flex items-center justify-center bg-slate-200 dark:bg-slate-700">
          <FolderIcon className="w-7 h-7 text-slate-400" />
        </div>
      </div>

      <h3 className="font-semibold text-lg text-slate-600 dark:text-slate-300 mb-1">
        Sin Categoría
      </h3>
